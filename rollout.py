...
class ROLLOUT(object):
    ...
    def get_reward(self, sess, input_x, rollout_num, discriminator):
        ...
        for i in range(rollout_num):
            ...
            # for given_num in range(2, self.sequence_length ):
            for given_num in range(2, self.sequence_length):

                feed = {self.x: input_x, self.given_num: given_num,self.start_token:input_x[:,0]}
                samples = sess.run(self.gen_x, feed)
                samplet_rollout.append(samples)
            ...
            if i == 0:
                ...
                rewards[self.sequence_length - 2] += ypred

        rewards = np.transpose(np.array(rewards)) / (1.0 * rollout_num)  # batch_size x seq_length
        return rewards,samplet_rollout
