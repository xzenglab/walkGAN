...
def main():
  ...
  for epoch in range(0,14,2):
      labels_matrix = mat[args.label_matrix_name]
      embeddings_file = 'rep_419/pre_gen_rep_{}.txt'.format(epoch)
      ...
      features_matrix = pd.read_table(embeddings_file, sep='\t', header=None)
      features_matrix = features_matrix.sort_values(by=0)

      features_matrix = features_matrix.values[:, 1:]
      ...
      print ('epoch:',epoch,'Results, using embeddings of dimensionality', X.shape[1])
      print ('-------------------')
